...
//configuring ethers
const contractAddress = '0x2d2813d8d40df5f7c9cd9fd6226a0cc3d502ca50';
const contractABI = [
    {
      "inputs": [],
...
const getAllCandidates = async function(){
    ...
    let candidates = await contract.retriveVotes();
...
const sendVote = async function(){
    let electionStarted = await contract.electionStarted();
    if (!electionStarted) {
        alert('No active election currently. Please wait for an election to start.');
        return;
    }
...
const addCandidate = async function() {
    ...
    await contract.addCandidate(candidates.value);
    refreshPage();
    candidates.value = "";
}
